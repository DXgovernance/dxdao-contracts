...
contract DXDGuild is ERC20Guild, OwnableUpgradeable {
    ...
    mapping(bytes32 => VotingMachineVoteProposal) public votingMachineVoteProposals;
    ...
    function createProposal(
        ...
        require(votingPowerOf(msg.sender) >= getVotingPowerForProposalCreation(), "DXDGuild: Not enough tokens to create proposal");
        require(
    ...
    function createVotingMachineVoteProposal(
      ...
      proposalsForVotingMachineVote[votingMachineVoteProposals[votingMachineProposalId].positiveVote] = votingMachineProposalId;
      ...
      proposalsForVotingMachineVote[votingMachineVoteProposals[votingMachineProposalId].negativeVote] = votingMachineProposalId;
    }
    
    /// @dev End positive and negative proposals to vote on a voting machine, executing the one with the higher vote 
    /// count first.
    /// @param votingMachineProposalId the proposalId of the voting machine
    function endVotingMachineVoteProposal(
        bytes32 votingMachineProposalId
    ) public isInitialized {
      Proposal storage positiveVote = proposals[votingMachineVoteProposals[votingMachineProposalId].positiveVote];
      Proposal storage negativeVote = proposals[votingMachineVoteProposals[votingMachineProposalId].negativeVote];
      require(positiveVote.state == ProposalState.Submitted, "DXDGuild: Positive proposal already executed");
      require(negativeVote.state == ProposalState.Submitted, "DXDGuild: Negative proposal already executed");
      require(positiveVote.endTime < block.timestamp, "DXDGuild: Positive proposal hasnt ended yet");
      require(negativeVote.endTime < block.timestamp, "DXDGuild: Negative proposal hasnt ended yet");
      
      if (positiveVote.totalVotes > negativeVote.totalVotes) {
        _endProposal(votingMachineVoteProposals[votingMachineProposalId].positiveVote);
        negativeVote.state = ProposalState.Rejected;
        emit ProposalRejected(votingMachineVoteProposals[votingMachineProposalId].negativeVote);
      } else {
        _endProposal(votingMachineVoteProposals[votingMachineProposalId].negativeVote);
        positiveVote.state = ProposalState.Rejected;
        emit ProposalRejected(votingMachineVoteProposals[votingMachineProposalId].positiveVote);
      }
    }
    ...
    function _setVote(address voter, bytes32 proposalId, uint256 votingPower) override internal isInitialized {
        ...
        if (votingMachineVoteProposalId != bytes32(0)) {
          require(getProposalVotesOfVoter(
            votingMachineVoteProposals[votingMachineVoteProposalId].positiveVote ,msg.sender
          ) == 0, "DXDGuild: Already voted in VotingMachine vote proposal");
          require(getProposalVotesOfVoter(
            votingMachineVoteProposals[votingMachineVoteProposalId].negativeVote ,msg.sender
          ) == 0, "DXDGuild: Already voted in VotingMachine vote proposal");
        }
        if (votingPower > proposals[proposalId].votes[voter]) {
            ...
            emit VoteAdded(
                proposalId, voter, votingPower.sub(proposals[proposalId].votes[voter])
            );
        } else {
            ...
            emit VoteRemoved(
                proposalId, voter, proposals[proposalId].votes[voter].sub(votingPower)
            );
        }
