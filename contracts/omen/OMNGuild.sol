...
contract OMNGuild is ERC20Guild {
    ...
    event GuildProposalCreated(uint256 indexed guildProposalId);
    event GuildProposalExecuted(uint256 indexed guildProposalId);
    ...
    {
        require(
            votesOf(msg.sender) >= getVotesForCreation(),
    ...
    function endMarketValidationProposal(bytes32 questionId) public {
        bytes32 marketValidProposalId =
            marketValidationProposals[questionId].marketValid;
        bytes32 marketInvalidProposalId =
            marketValidationProposals[questionId].marketInvalid;
        _endValidationProposal(marketValidProposalId, marketInvalidProposalId);
    }

    function _endValidationProposal(bytes32 validId, bytes32 invalidId)
        private
    {
    ...
    function endProposal(bytes32) public pure override {
    ...
    function endGuildProposal(uint256 guildProposalId) public {
        bytes32 guildValidProposalId = guildProposals[guildProposalId].Valid;
        bytes32 guildInvalidProposalId =
            guildProposals[guildProposalId].Invalid;
    ...
    function setPositiveVote(uint256 guildProposalId, uint256 amount) public {
    ...
    function setVote(bytes32 proposalId, uint256 amount)
        public
        virtual
        override
    {
        ...
        require(
            ...
            "OMNGuild: Already voted on proposal"
        );
        require(
            proposals[
                marketValidationProposals[
    ...
    function setVotes(bytes32[] memory proposalIds, uint256[] memory amounts)
        public
        virtual
        override
    {
        ...
        for (uint256 i = 0; i < proposalIds.length; i++) {
            require(
                votesOfAt(msg.sender, proposals[proposalIds[i]].snapshotId) >=
                    amounts[i],
                "OMNGuild: Invalid amount"
            );
            require(
                proposals[proposalIds[i]].votes[msg.sender] == 0,
                "OMNGuild: Already voted on this proprosal"
            );
            require(
                proposals[
                    marketValidationProposals[
                        proposalsForMarketValidation[proposalIds[i]]
                    ]
                        .marketValid
                ]
                    .votes[msg.sender] == 0,
            ...
            require(
                amounts[i] <= maxAmountVotes,
                "OMNGuild: Cant vote with more votes than max amount of votes"
            );
            if (amounts[i] > 0) {
                positiveVotesCount[proposalIds[i]] = positiveVotesCount[
                    proposalIds[i]
                ]
                    .add(1);
            }
    ...
    function getVotesForCreation() public view override returns (uint256) {
        return votesForCreation;
    }

    /// @dev Get minimum amount of votes needed for proposal execution
    function getVotesForExecution() public view override returns (uint256) {
    ...
    function createGuildProposal(
    ...
    ) public virtual isInitialized returns (uint256) {
        ...
        if (specialProposerPermissions[msg.sender].exists) {
            ...
            votesForCreation = specialProposerPermissions[msg.sender]
                .votesForCreation;
        }

        guildProposalCnt += 1;

        guildProposals[guildProposalCnt].Valid = super.createProposal(
            to,
            data,
            value,
            description,
        ...
        guildProposals[guildProposalCnt].Invalid = super.createProposal(
            to,
            noop,
            value,
            description,
        ...
        proposalsForGuild[
            guildProposals[guildProposalCnt].Valid
        ] = guildProposalCnt;
        proposalsForGuild[
            guildProposals[guildProposalCnt].Invalid
        ] = guildProposalCnt;
