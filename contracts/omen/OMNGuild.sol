...
contract OMNGuild is ERC20Guild {
  ...
  function initialize(
    ...
    callPermissions[address(this)][
      bytes4(keccak256('setOMNGuildConfig(uint256,address,uint256,uint256)'))
    ] = block.timestamp;
    callPermissions[address(this)][
      bytes4(keccak256('setSpecialProposerPermission(address,uint256,uint256)'))
    ] = block.timestamp;
  }
  ...
  function claimMarketValidationVoteRewards(
    ...
    for (uint256 i = 0; i < proposalIds.length; i++) {
      ...
      if (
        ...
        reward = reward.add(
          unsuccessfulVoteReward.div(positiveVotesCount[proposalIds[i]])
        );
      }
    ...
    if (marketValidationProposalId != bytes32(0)) {
      require(
        getProposalVotesOfVoter(
          marketValidationProposals[marketValidationProposalId].marketValid,
          msg.sender
        ) == 0,
      ...
      require(
        getProposalVotesOfVoter(
          marketValidationProposals[marketValidationProposalId].marketInvalid,
          msg.sender
        ) == 0,
        'OMNGuild: Already voted in marketValidation proposal'
      );
  ...
  function createProposal(
    ...
    if (specialProposerPermissions[msg.sender].exists) {
      ...
      votingPowerForProposalCreation = specialProposerPermissions[msg.sender]
