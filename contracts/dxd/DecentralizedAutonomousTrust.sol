...
contract DecentralizedAutonomousTrust is ERC20, ERC20Detailed {
    ...
    event Buy(address indexed _from, address indexed _to, uint256 _currencyValue, uint256 _fairValue);
    event Sell(address indexed _from, address indexed _to, uint256 _currencyValue, uint256 _fairValue);
    event Burn(address indexed _from, uint256 _fairValue);
    event Pay(address indexed _from, address indexed _to, uint256 _currencyValue, uint256 _fairValue);
    ...
    mapping(address => uint256) public initInvestors;
    ...
    function _transfer(
        ...
        address _to,
        uint256 _amount
    ) internal authorizeTransfer(_from, _to, _amount, false) {
    ...
    function _mint(address _to, uint256 _quantity) internal authorizeTransfer(address(0), _to, _quantity, false) {
    ...
    function updateConfig(
        ...
        if (beneficiary != _beneficiary) {
            ...
            uint256 tokens = balanceOf(beneficiary);
    ...
    function _distributeInvestment(uint256 _value) private {
        ...
        uint256 reserve = investmentReserveBasisPoints.mul(_value);
        reserve /= BASIS_POINTS_DEN;
        reserve = _value.sub(reserve);
        uint256 fee = reserve.mul(feeBasisPoints);
    ...
    function estimateBuyValue(uint256 _currencyValue) public view returns (uint256) {
        ...
        uint256 tokenValue;
        if (state == STATE_INIT) {
            uint256 currencyValue = _currencyValue;
            ...
            uint256 supply = totalSupply() + burnedSupply - initReserve;
            // Math: worst case
            // MAX * 2 * MAX_BEFORE_SQUARE
            // / MAX_BEFORE_SQUARE
    ...
    function buy(
        ...
        if (state == STATE_INIT) {
            ...
            if (totalSupply() + tokenValue - initReserve >= initGoal) {
                ...
                // / MAX_BEFORE_SQUARE * 2
                uint256 beneficiaryContribution = BigDiv.bigDiv2x1(
    ...
    function estimateSellValue(uint256 _quantityToSell) public view returns (uint256) {
        ...
        if (state == STATE_RUN) {
            ...
            // Math worst case:
            // MAX * 2 * MAX_BEFORE_SQUARE
            uint256 temp = _quantityToSell.mul(2 * reserve);
            temp /= supply;
    ...
    function estimatePayValue(uint256 _currencyValue) public view returns (uint256) {
        ...
        uint256 tokenValue = BigDiv.bigDiv2x1(
    ...
    function _pay(address _to, uint256 _currencyValue) private {
        ...
        uint256 reserve = _currencyValue.mul(investmentReserveBasisPoints);
        reserve /= BASIS_POINTS_DEN;
