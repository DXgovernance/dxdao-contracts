...
contract VotingMachine {
     ...
     *      to enable users to redeem on behalf of someone else.
     ...
     * Every proposal has a unique ID which is being generated by calculating keccak256 of a incremented counter.
     ...
     * @param amount How much voting power the voter stakes for this vote. if amount==0, the max voting power is used.
     * @return proposalExecuted True if the proposal was executed, false otherwise.
    ...
    function _execute(bytes32 proposalId) internal votable(proposalId) returns (bool proposalExecuted) {
        ...
        if (proposal.executionState != ExecutionState.None) {
            ...
            emit ExecuteProposal(proposalId, schemes[proposal.schemeId].avatar, proposal.winningVote, totalVotingPower);
