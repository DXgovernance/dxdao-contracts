...
contract("Controller", accounts =>  {
  ...
  it("register schemes - check permissions for updating existing scheme", async() => {
    ...
    let tx = await controller.registerScheme(
      accounts[ 0 ], "0x0000000000000000000000000000000000000000", "0x00000001", avatar.address
    );
    assert.equal(tx.logs.length, 1);
    assert.equal(tx.logs[ 0 ].event, "RegisterScheme");
    ...
    try {
      await controller.registerScheme(
        accounts[ 0 ], "0x0000000000000000000000000000000000000000", "0x00000002", avatar.address
      );
      assert(false, 
  ...
  it("addGlobalConstraint ", async() => {
    ...
    var tx = await controller.addGlobalConstraint(
      globalConstraints.address, "0x0000000000000000000000000000000000000000", avatar.address
    );
