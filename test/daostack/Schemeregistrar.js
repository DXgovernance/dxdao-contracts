...
const setupSchemeRegistrarParams = async function (schemeRegistrar) {
  ...
  const paramsHash = await schemeRegistrar.getParametersHash(
    votingMachine.params,
    votingMachine.params,
    votingMachine.address
  );
...
const setup = async function (accounts) {
  ...
  const schemeRegistrarParams = await setupSchemeRegistrarParams(
    schemeRegistrar
  );
...
contract("SchemeRegistrar", accounts => {
  ...
  it("execute proposeScheme  and execute -yes - fee > 0 ", async function () {
    ...
    await testSetup.schemeRegistrarParams.votingMachine.contract.vote(
      proposalId,
      1,
      0,
      constants.NULL_ADDRESS,
