...
const setupContributionRewardParams = async function (
  ...
  if (votingMachineType !== "none") {
    votingMachine = await helpers.setUpVotingMachine(token);
...
const setup = async function (
  ...
  await org.daoCreator.setSchemes(
    ...
    [contributionRewardParams.paramsHash],
    [permissions],
    "metaData"
  );
  return {
...
contract("ContributionReward", accounts => {
  ...
  it("proposeContributionReward log", async function () {
    ...
    assert.equal(
      await helpers.getValueFromLogs(tx, "_avatar", 0),
      testSetup.org.avatar.address,
      "Wrong log: _avatar"
    );
    assert.equal(
      await helpers.getValueFromLogs(tx, "_intVoteInterface", 0),
      testSetup.contributionRewardParams.votingMachine.address,
      "Wrong log: _intVoteInterface"
    );
    assert.equal(
      await helpers.getValueFromLogs(tx, "_descriptionHash", 15),
      "description-hash",
      "Wrong log: _contributionDescription"
    );
    assert.equal(
      await helpers.getValueFromLogs(tx, "_reputationChange", 0),
      10,
      "Wrong log: _reputationChange"
    );
    ...
    assert.equal(arr[4].words[0], 1, "Wrong log: _rewards");
    assert.equal(
      await helpers.getValueFromLogs(tx, "_externalToken", 0),
      testSetup.standardTokenMock.address,
      "Wrong log: _externalToken"
    );
    assert.equal(
      await helpers.getValueFromLogs(tx, "_beneficiary", 0),
      accounts[0],
      "Wrong log: _beneficiary"
    );
  });
  ...
  it("execute proposeContributionReward to expensive receiver fallback function will fail in redeem", async function () {
    ...
    try {
      ...
      assert.equal(e, "Error: Transaction reverted without a reason string");
    }
  ...
  it("should fail if trying to submit periodsLength > 1 or == 0", async function () {
    var testSetup = await setup(accounts);
    ...
    try {
      ...
      assert.equal(
        ex.message,
        "VM Exception while processing transaction: reverted with reason string 'numberOfPeriods should be 1'"
      );
    }
    ...
    try {
      await testSetup.contributionReward.proposeContributionReward(
        ...
        web3.utils.asciiToHex("description"),
        reputationReward,
        [nativeTokenReward, ethReward, 0, periodLength, 2],
        testSetup.standardTokenMock.address,
        accounts[1]
      );
    } catch (ex) {
  ...
  it("execute proposeContributionReward via genesisProtocol voting machine and redeem using Redeemer for un executed boosted proposal", async function () {
    ...
    await standardTokenMock.approve(
      testSetup.contributionRewardParams.votingMachine.contract.address,
      1000,
      { from: accounts[0] }
    );
    const stakeTx =
  ...
  it("execute proposeContributionReward  param validate ", async function () {
    ...
    try {
      ...
      assert(
        false,
        "numberOfPeriods * externalTokenReward should not overflow"
      );
    } catch (ex) {
      helpers.assertVMException(ex);
    }
