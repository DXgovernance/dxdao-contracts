...
contract("OMNGuild", function (accounts) {
  ...
  beforeEach(async function () {
    ...
    await omnGuild.setPositiveVote(guildProposalId, 40, {
      from: accounts[4],
    });
    ...
    questionId = (
      await realitio.askQuestion(
        0 /* template_id */,
        "Is market with [questionID] valid?",
        omnGuild.address,
        60 * 60 * 24 * 2 /* timeout, */,
        latest /* opening_ts */,
        0 /* nonce */
  ...
  describe("OMNGuild use tests", function () {
    it("vote on and execute a market validation proposal from the omn-guild", async function () {
      ...
      await expectRevert(
        omnGuild.setVote(marketValidationProposalValid, 999999, {
          from: accounts[4],
        }),
        "OMNGuild: Invalid amount"
      );
      await expectRevert(
        omnGuild.setVote(marketValidationProposalValid, 100, {
          from: accounts[4],
        }),
    ...
    it(msgD, async function () {
      await expectRevert(omnGuild.endGuildProposal(guildProposalId), msgD);
    });
    ...
    it("test changing vote I.B.3.c: Voters CANNOT change vote once they've voted", async function () {
      ...
      await expectRevert(
        omnGuild.setVote(marketValidationProposalValid, 1, {
          from: accounts[4],
        }),
        "OMNGuild: Already voted"
      );
    ...
    it("test changing vote I.B.3.c: Voters CANNOT change vote once they've voted", async function () {
      const txVote = await omnGuild.setVote(marketValidationProposalValid, 1, {
        from: accounts[4],
      });
    ...
    it("claim rewards for successful vote", async function () {
      ...
      await expectRevert(
        omnGuild.claimMarketValidationVoteRewards(
          [marketValidationProposalValid],
          accounts[4]
        ),
    ...
    it("test setVotes prevents voting twice", async function () {
      await expectRevert(
        omnGuild.setVotes(
          [marketValidationProposalInvalid, marketValidationProposalInvalid],
          [10000, 999, 888],
          { from: accounts[4] }
        ),
      ...
      await expectRevert(
        omnGuild.setVotes(
          [marketValidationProposalInvalid, marketValidationProposalValid],
          [10, 9],
          { from: accounts[3] }
        ),
        "OMNGuild: Already voted"
      ...
      await expectRevert(
        omnGuild.setVotes(
          [marketValidationProposalValid, marketValidationProposalValid],
          [10, 9],
          { from: accounts[3] }
        ),
        "OMNGuild: Already voted"
    ...
    it("test setVotes prevents changing vote", async function () {
      await expectRevert(
        omnGuild.setVotes(
          [marketValidationProposalValid, marketValidationProposalInvalid],
          [10, 9],
          { from: accounts[3] }
        ),
        "OMNGuild: Already voted"
    ...
    it("test createProposal", async function () {
      const testData = await new web3.eth.Contract(OMNGuild.abi).methods
        .getVotesForExecution()
        .encodeABI();
      await expectRevert(
    ...
    it("test createGuildProposal", async function () {
      ...
      const testProposal = helpers.getValueFromLogs(
        tx,
        "guildProposalId",
        "GuildProposalCreated"
      );
      ...
      await omnGuild.setPositiveVote(setAllowanceProposalId, 40, {
        from: accounts[4],
      });

      await time.increase(time.duration.seconds(60 * 60 * 24 * 7 + 1000));
      await expectRevert(
        omnGuild.endGuildProposal(testProposal),
        "Not allowed call"
      );
      const setAllowanceReceipt = await omnGuild.endGuildProposal(
      ...
      await time.increase(time.duration.seconds(60 * 60 * 24 * 7 + 1000));
      const receipt = await omnGuild.endGuildProposal(
        setSpecialProposerPermissionProposalId
      );
      ...
      const testProposal2 = helpers.getValueFromLogs(
        tx2,
        "guildProposalId",
        "GuildProposalCreated"
      );
      await omnGuild.setPositiveVote(testProposal2, 40, {
        from: accounts[4],
      });
      ...
      const receiptForTestPropsal2 = await omnGuild.endGuildProposal(
        testProposal2
      );
      expectEvent(receiptForTestPropsal2, "GuildProposalExecuted", {
