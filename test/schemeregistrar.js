...
const setupSchemeRegistrarParams = async function(schemeRegistrar) {
  const votingMachine = await helpers.setupAbsoluteVote(helpers.NULL_ADDRESS,50,schemeRegistrar.address);
...
contract('SchemeRegistrar', accounts => {
  ...
  it("execute proposeScheme  and execute -yes - fee > 0 ", async function() {
    ...
    await testSetup.schemeRegistrarParams.votingMachine.contract.vote(proposalId,1,0,helpers.NULL_ADDRESS,{from:accounts[2]});
    var controller = await Controller.at(await testSetup.org.avatar.owner());
    assert.equal(await controller.isSchemeRegistered(helpers.SOME_ADDRESS,testSetup.org.avatar.address),true);
  });
  ...
  it("execute proposeScheme - no decision (same for remove scheme) - proposal data delete", async function() {
    ...
    //Vote with reputation to trigger execution
    await testSetup.schemeRegistrarParams.votingMachine.contract.vote(proposalId,2,0,helpers.NULL_ADDRESS,{from:accounts[2]});
    var controller = await Controller.at(await testSetup.org.avatar.owner());
  ...
  it("execute proposeScheme  and execute -yes - autoRegisterOrganization==TRUE arc scheme", async function() {
    ...
    var tx = await testSetup.schemeRegistrar.proposeScheme(testSetup.org.avatar.address,helpers.SOME_ADDRESS,helpers.NULL_HASH,"0x00000000",helpers.NULL_HASH);
    //Vote with reputation to trigger execution
    var proposalId = await helpers.getValueFromLogs(tx, '_proposalId',1);
    await testSetup.schemeRegistrarParams.votingMachine.contract.vote(proposalId,1,0,helpers.NULL_ADDRESS,{from:accounts[2]});
  });
