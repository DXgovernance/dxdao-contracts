...
task("create2", "Deploy a smart contract using create2")
  ...
  .setAction(async ({contract, salt, initializer}) => {
    ...
    const create2DeployFactory = contracts[hre.network.name].utils.create2Deployer;
    ...
    if (codeInAddress.length > 2) {
      ...
      const create2Deployer = await hre.artifacts.require("Create2Deployer").at(
        contracts[hre.network.name].utils.create2Deployer
      );
     
      const deployTx = await create2Deployer.deploy(toBeDeployed.bytecode, salt, {from: deployerAccount});
      ...
      if (initializer) {
        ...
        await deployedContract.initialize(...initializer.split(','), { from: deployerAccount });
      }
    }
  });
